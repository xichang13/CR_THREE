...
| stosb | 1. store string <br> 2. 将al或ax的值存储到di地址 <br> 3. (di) <- al或ax | di <- di+1 | di <- di-1 |
| stosw | 1. store string <br> 2. 将al或ax的值存储到di地址 <br> 3. (di) <- al或ax | di <- di+2 | di <- di-2 |
| lodsb | 1. load string <br> 2. 将si地址的内容拷贝到al或ax <br> 3. al或ax <- (si) | si <- si+1 | si <- si-1 |
| lodsw | 1. load string <br> 2. 将si地址的内容拷贝到al或ax <br> 3. al或ax <- (si) | si <- si+2 | si <- si-2 |
| cmpsb | 1. compare string <br> 2. si地址内容减去di地址内容，不存储结果，影响标志位 <br> 3. (si) - (di) | si <- si+1 <br> di <- di+1 | si <- si-1 <br> di <- di-1 |
| cmpsw | 1. compare string <br> 2. si地址内容减去di地址内容，不存储结果，影响标志位 <br> 3. (si) - (di) | si <- si+2 <br> di <- di+2 | si <- si-2 <br> di <- di-2 |
| scasb | 1. scan string <br> 2. al或ax减去di地址内容，不存储结果，影响标志位 <br> 3. al或ax - (di) | di <- di+1 | di <- di-1 |
| scasw | 1. scan string <br> 2. al或ax减去di地址内容，不存储结果，影响标志位 <br> 3. al或ax - (di) | di <- di+2 | di <- di-2 |
